...
export const TICKET_BUYING_RESEARCH_SUMMARY = {
    status: "wip",
    safety: "DBhopper ticket buying is dry-run only. It may open DB's booking website, but it must not submit payment or finalize a purchase.",
    purchaseCandidates: [
        {
            name: "bahn.de/int.bahn.de browser flow",
            status: "best initial path",
            notes: "Official consumer path. Works with or without DB account; automation should stop before payment unless a later explicit confirmation gate exists.",
        },
        {
...
export function ticketCheckoutPlan(params, userDataDir, now = new Date()) {
...
async function runBrowserTicketCheckout(params, config, plan, loadedCredentials, signal) {
    const artifactDir = await createTicketArtifactDir(config);
    const artifacts = [];
    let browser;
    let context;
...
async function detectCheckoutBoundary(page) {
    ...
    const paymentBoundaryVisible = /payment\s+method|payment\s+details|zahlungsart|zahlungsdaten|pay\s+with|credit\s+card|kreditkarte|paypal|sepa|card\s+number|kartennummer/i.test(body) || await page.locator('input[name*="card" i], input[id*="card" i], input[name*="iban" i], input[id*="iban" i]').first().isVisible({ timeout: 500 }).catch(() => false);
    return {
