...
import { assertKnownKeys, assertNumericIdString, assertString, assertTable, } from "./schema-helpers.js";
...
export const BUYING_FARE_PRODUCT_LABELS = {
    super_sparpreis: "Super Sparpreis",
    sparpreis: "Sparpreis",
    flexpreis: "Flexpreis",
    cheapest_available: "Cheapest available offer",
};
export const BUYING_TRAVEL_CLASS_LABELS = {
    second: "2. Klasse",
    first: "1. Klasse",
};
const FARE_PRODUCT_ALIASES = new Map([
...
const TRAVEL_CLASS_ALIASES = new Map([
    ["second", "second"],
    ["2", "second"],
    ["2nd", "second"],
    ["2. klasse", "second"],
...
const BOOKING_FOR_ALIASES = new Map([
    ["self", "self"],
    ["me", "self"],
    ["for me", "self"],
    ["book for me", "self"],
...
export async function readSelectedBuyingProfile(config = {}) {
    const selected = await readSelectedPrivateToml(config, resolveSelectedBuyingProfileFile, parseBuyingProfileToml);
    if (!selected) {
        return undefined;
    }
    return {
...
export function resolveBuyingFarePreference(profile) {
    ...
    const fallbackFares = profile?.fallbackFares?.length
        ...
        : BUYING_FARE_PRODUCT_ORDER.filter((fare) => fare !== defaultFare && fare !== "cheapest_available");
...
export function fareProductLabel(value) {
    return BUYING_FARE_PRODUCT_LABELS[value];
}
export function travelClassLabel(value) {
    return BUYING_TRAVEL_CLASS_LABELS[value];
}
function normalizeBuyingProfile(value) {
...
function uniqueFareOrder(values) {
    const seen = new Set();
    const result = [];
...
function assertBuyingProfileShape(value, source) {
...
function normalizeFareProduct(value, source) {
    const normalized = FARE_PRODUCT_ALIASES.get(value.trim().toLowerCase());
    if (!normalized) {
        throw new Error(`${source} must be one of: ${BUYING_FARE_PRODUCT_ORDER.join(", ")}`);
    }
    return normalized;
}
function normalizeTravelClass(value, source) {
...
function normalizeBookingFor(value, source) {
