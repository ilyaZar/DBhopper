...
export async function writePrivateSettingsRuntimeConfig(updates, config = {}) {
    ...
    const updated = {
        ...
        TEST_RUN_CLAIM_REQUEST: normalized.test_run_claim_request ?? loaded.settings.TEST_RUN_CLAIM_REQUEST,
        TEST_RUN_PURCHASE: normalized.test_run_purchase ?? loaded.settings.TEST_RUN_PURCHASE,
        CLAIM_REQUEST_MODE: normalized.claim_request_mode ?? loaded.settings.CLAIM_REQUEST_MODE,
...
function assertPrivateSettingsShape(value, source) {
    ...
    assertOneOf(value.CLAIM_REQUEST_MODE, CLAIM_REQUEST_MODE_VALUES, `${source}.claim_request_mode`);
...
function normalizeClaimRequestMode(value, source) {
...
function normalizeRuntimeConfigUpdates(updates) {
    ...
    if ("claim_request_mode" in updates) {
        normalized.claim_request_mode = normalizeClaimRequestMode(updates.claim_request_mode, "claim_request_mode");
    }
...
function claimRequestModeMeaning(value) {
    switch (value) {
        case "review":
