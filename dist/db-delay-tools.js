...
export async function runDbDelayQuery(params, config = {}, signal) {
    const status = timetablesConfigStatus(config);
    const providerChoice = selectDelayProvider(params.provider, config, status.configured);
    ...
    try {
        const provider = providerChoice.selected === BAHN_WEB_SOURCE_API
            ? createBahnWebProvider({
                ...config,
                signal,
                timeZone: params.time_zone ?? config.timeZone,
            })
        ...
        return {
            ...
            needs_configuration: /credentials/i.test(error instanceof Error ? error.message : String(error)),
            provider_selection: providerChoice,
            research: combinedResearchSummary(),
        };
    }
...
function selectDelayProvider(requested, config, timetablesConfigured) {
