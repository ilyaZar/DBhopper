...
export function claimSchemaReference() {
  return {
    ...
    submittedRecipeShape:
      "claim_submitted_recipe.toml records submission status without private claim data.",
  };
}
  ...
  if (journey.delayMinutes === undefined || journey.delayMinutes === null) {
    messages.push(
      warning(
        ...
        "delayMinutes is missing; browser filing can continue but eligibility is not proven",
      ),
    );
