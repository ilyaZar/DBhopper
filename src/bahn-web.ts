...
export const DEFAULT_BAHN_WEB_BASE_URL =
  "https://int.bahn.de/web/api";
export const BAHN_WEB_FALLBACK_BASE_URL =
...
type RequestTransport = Exclude<BahnWebTransport, "auto">;

interface BahnWebRequestOptions extends BahnWebProviderOptions {
  baseUrl: string;
}
...
export function createBahnWebProvider(
  options: BahnWebProviderOptions = {},
): ApiProvider {
  return {
    resolveStation: (name) => resolveBahnWebStation(name, options),
    queryStationBoard: (station, timeWindow) =>
      queryBahnWebStationBoard(station, timeWindow, options),
    fetchJourneyDetails: fetchBahnWebJourneyDetails,
  };
}

export async function resolveBahnWebStation(
  name: string,
  options: BahnWebProviderOptions = {},
) {
...
export async function queryBahnWebStationBoard(
  station: StationRef,
  ...
  const responses = await Promise.all(
    anchors.map((anchor) =>
      fetchBahnWebJson(
        ...
        buildDepartureParams(station.evaNo ?? "", anchor.date, anchor.time),
        options,
      ),
    ),
  );

  return dedupeStationEvents(
    responses.flatMap((response) =>
      parseBahnWebDepartures(response, station, timeZone),
    ),
  );
}

export async function fetchBahnWebJourneyDetails(event: StationEvent) {
  if (!event.journey) {
    throw new Error(
      `Bahn web event ${event.journeyId ?? event.label ?? "unknown"} has no journey details`,
    );
  }
  return event.journey;
}

export function parseBahnWebStations(raw: unknown): StationRef[] {
  return toArray(raw)
    .map((entry) => stationRefFromBahnWeb(entry))
    .filter((station): station is StationRef => Boolean(station));
}

export function parseBahnWebDepartures(
  raw: unknown,
...
async function fetchBahnWebJson(
  path: string,
...
async function fetchJsonByTransport(
  url: string,
  ...
  if (result.status < 200 || result.status >= 300) {
    throw new Error(
      `HTTP ${result.status}: ${result.body.slice(0, 200).replace(/\s+/g, " ")}`,
    );
  }
  try {
...
function resolveTransports(transport?: BahnWebTransport): RequestTransport[] {
...
function normalizeAllowedBahnWebBaseUrl(value: string) {
  const url = new URL(value);
  if (url.protocol !== "https:" || !ALLOWED_HOSTS.has(url.hostname)) {
    throw new Error(
      "bahnWebBaseUrl must use https://int.bahn.de or https://www.bahn.de",
    );
  }
...
function buildDepartureParams(evaNo: string, date: string, time: string) {
...
function stationRefFromBahnWeb(raw: unknown): StationRef | null {
...
function stationEventFromBahnWebEntry(
  raw: unknown,
  boardStation: StationRef,
  timeZone: string,
): StationEvent | null {
...
function parseBahnWebTimestamp(value: string | undefined, timeZone: string) {
  if (!value) {
    return undefined;
  }
...
function uniqueAnchors(dates: Date[], timeZone: string) {
  const anchors = new Map<string, { date: string; time: string }>();
...
function formatBahnWebLocalDateTime(date: Date, timeZone: string) {
...
function labelFromProduct(product: Record<string, unknown> | undefined, category: string) {
  return (
    stringAttr(product, "mittelText") ??
    stringAttr(product, "name") ??
    stringAttr(product, "linienNummer") ??
...
function dedupeStationEvents(events: StationEvent[]) {
  const deduped: StationEvent[] = [];
...
function sameStationEvent(left: StationEvent, right: StationEvent) {
...
function compareEventsByPlannedTime(left: StationEvent, right: StationEvent) {
  return (
    Date.parse(left.plannedDeparture ?? left.realtimeDeparture ?? "") -
...
function objectAttr(value: unknown): Record<string, unknown> | undefined {
...
function fieldAttr(value: unknown, key: string) {
  return objectAttr(value)?.[key];
}
...
function stringArrayAttr(value: Record<string, unknown> | undefined, key: string) {
  const attr = value?.[key];
...
function boolAttr(value: Record<string, unknown>, key: string) {
  const attr = value[key];
